...
const currentLanguage = i18n.language;  const [flowerData, setFlowerData] = useState([]);
  ...
  const fetchHerbs = async () => {
    ...
    try {
      ...
      // Fetch all herbs
      const allHerbs = await apiFetch({
        endpoint: `/herbalVault/?lang=${currentLanguage}`,
        auth: true
      });
      ...
      // Fetch featured herb
      const featuredData = await apiFetch({
        endpoint: `/daily-featured-herb/?lang=${currentLanguage}`,
        auth: true
      });
  ...
  if (loading) {
    return (
      <View style={[styles.container, { justifyContent: "center", alignItems: "center" }]}>
        ...
        <Text style={{ marginTop: 10, color: "#4a6a3b" }}>{t("loadingHerbs")}</Text>
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{t("herbalVault")}</Text>
        </View>
          ...
          ListEmptyComponent={
            gridData.length === 0 && (
              <View style={{ flex: 1, alignItems: "center", marginTop: 20 }}>
                <Text style={{ color: "grey", fontSize: 16 }}>{t("noResultsFound")}</Text>
              </View>
            )
          }
        />
